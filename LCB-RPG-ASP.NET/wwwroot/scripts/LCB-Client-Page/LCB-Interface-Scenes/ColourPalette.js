...
// @ts-ignore
import { EffectPass, RenderPass, KernelSize, FXAAEffect, Selection, SMAAEffect, SMAAPreset, BlendFunction, EdgeDetectionMode, PredicationMode, TextureEffect, GlitchEffect, ChromaticAberrationEffect } from "postprocessing";
//Custom PostProcessing
import { SelectiveBloomEffect, EffectComposer as PostProcessingEffectComposer, NoiseEffect } from '../../../lib/local/customPostprocessing.js';
...
export function RenderPage() {
    ...
    let emergencyOrangeGeometries = new Array;
    let warningRedGeometries = new Array;
    let alertYellowGeometries = new Array;
    let deathBlackGeometries = new Array;
    let retroWhiteGeometries = new Array;
    let conditionBlueGeometries = new Array;
    let magiOrangeGeometries = new Array;
...
function RunRender(composer) {
    requestAnimationFrame(function render() {
        requestAnimationFrame(render);
        composer.render();
    });
}
function GenerateStaticGeometries(scene) {
    //Geometries
...
function NewMatrixGreenGeometries(scene) {
...
function NewConditionBlueGeometries(scene) {
    const objectList = new Array;
    //Geometries
    const solidBox = new THREE.BoxGeometry(1, 1, 1);
    const boxOutline = new THREE.EdgesGeometry(solidBox);
...
function NewMAGIOrangeGeometries(scene) {
    const objectList = new Array;
    //Geometries
    const solidBox = new THREE.BoxGeometry(1, 1, 1);
    const boxOutline = new THREE.EdgesGeometry(solidBox);
...
function NewAlertYellowGeometries(scene) {
    const objectList = new Array;
    //Geometries
    const solidBox = new THREE.BoxGeometry(1, 1, 1);
    const boxOutline = new THREE.EdgesGeometry(solidBox);
...
function NewRetroWhiteGeometries(scene) {
    const objectList = new Array;
    //Geometries
    const solidBox = new THREE.BoxGeometry(1, 1, 1);
    const boxOutline = new THREE.EdgesGeometry(solidBox);
...
function NewDeathBlackGeometries(scene) {
    const objectList = new Array;
    //Geometries
    const solidBox = new THREE.BoxGeometry(1, 1, 1);
    const boxOutline = new THREE.EdgesGeometry(solidBox);
...
function NewEmergencyOrangeGeometries(scene) {
    const objectList = new Array;
    //Geometries
    const solidBox = new THREE.BoxGeometry(1, 1, 1);
    const boxOutline = new THREE.EdgesGeometry(solidBox);
...
function NewWarningRedGeometries(scene) {
    const objectList = new Array;
    //Geometries
    const solidBox = new THREE.BoxGeometry(1, 1, 1);
    const boxOutline = new THREE.EdgesGeometry(solidBox);
...
function ApplyPostProcessing(composer, scene, camera, renderer, instructions, highestLayerUsed) {
...
function InitialiseEffectComposer(scene, camera, renderer) {
...
function ApplyBlooms(composer, scene, camera, instructions, currentLayer) {
    let bloomLayer = currentLayer + 1;
    let bloomedTargets = new Set;
    if (bloomLayer > 32) {
    ...
    instructions.forEach((currentInstruction) => {
        ...
        if (instructionName.startsWith("Bloom")) {
            ...
            composer.addPass(new EffectPass(camera, new SelectiveBloomEffect(scene, camera, new Selection(currentInstruction.Targets, bloomLayer++), { luminanceThreshold: 0, intensity: bloomIntensity, kernelSize: KernelSize.HUGE })));
...
function ApplyNoise(composer, camera) {
...
function ApplyFlicker(composer, camera) {
...
function ApplyChromaticAbberation(composer, camera) {
...
function ApplyAntiAliasing(composer, camera) {
    //FXAAx2
    ...
    // BEGIN DEBUG
    const smaaEdgesDebugPass = new EffectPass(camera, smaaeffect, new TextureEffect({ texture: smaaeffect.edgesTexture }));
    const smaaWeightsDebugPass = new EffectPass(camera, smaaeffect, new TextureEffect({ texture: smaaeffect.weightsTexture }));
