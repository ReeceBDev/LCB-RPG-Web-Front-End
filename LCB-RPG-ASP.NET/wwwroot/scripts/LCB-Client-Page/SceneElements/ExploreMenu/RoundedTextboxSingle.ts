...
export function RoundedBorderBox(height: number, width: number, colour: number, linewidth: number): THREE.Group{
    ...
    roundedBox.add(
    new Wireframe(
        new LineSegmentsGeometry().fromEdgesGeometry(new THREE.EdgesGeometry(borderGeometry)),
        new LineMaterial({ color: colour, linewidth: linewidth })
    ));

    return roundedBox;
}

export function RoundedTextboxSingle(height: number, width: number, colour: number, linewidth: number, text: string, textSize: number): THREE.Group{
    const roundedTextbox = new THREE.Group();
    const border: THREE.Group = RoundedBorderBox(height, width, colour, linewidth);
