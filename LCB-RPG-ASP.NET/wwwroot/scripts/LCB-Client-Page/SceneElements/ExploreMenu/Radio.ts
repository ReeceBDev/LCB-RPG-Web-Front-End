...
export class Radio extends SceneElement{
    private radioMaterial: THREE.Material;
    private radioAspectRatio: number;

    public override InitiatePreload(): void {
        const textureLoader = new THREE.TextureLoader();

        textureLoader.load(
            '/resources/radiotransparent.png',
            (texture) => {
    ...
    protected override GenerateElement(): THREE.Group {
        ...
        let radioProp: THREE.Mesh;
        let yellowRadioBackground: THREE.Mesh;
        let greyblueRadioBackground: THREE.Mesh;
        let topRadioText: Text;
        let bottomRadioTextpt1: Text;
        let bottomRadioTextpt2: Text;
