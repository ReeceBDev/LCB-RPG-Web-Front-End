...
export class Status extends SceneElement{
    protected override GenerateElement(): THREE.Group {
        ...
        let relativeTimeBackgroundPlane: THREE.PlaneGeometry;
        let relativeTimeBackgroundBox: THREE.Mesh;
        let relativeTimeBackgroundEdge: THREE.LineSegments;

        status.add(this.GenerateUserNameStatus());

        //Relative Time
        relativeTimeBackgroundPlane = new THREE.PlaneGeometry(20, 4);
        relativeTimeBackgroundEdge = new THREE.LineSegments(
            new THREE.EdgesGeometry(relativeTimeBackgroundPlane),
            new THREE.LineBasicMaterial({ color: Colour.MagiOrange, linewidth: 5 })

        );

        relativeTimeBackgroundBox = new THREE.Mesh(
            relativeTimeBackgroundPlane,
            new THREE.MeshBasicMaterial({ color: Colour.MatrixGreen, side: THREE.DoubleSide })
        );
    ...
    private GenerateUserNameStatus(): THREE.Group {
    ...
    private GenerateNameHarshBorder(): THREE.Group {
        ...
        //Background Panel
        statusBackgroundBox.add(
            new THREE.Mesh(
                new THREE.PlaneGeometry(26, 13),
                new THREE.MeshBasicMaterial({ color: Colour.DeathBlack, side: THREE.DoubleSide, opacity: 0.1, transparent: true })
            ));

        //Right Edge Corner
        upperRightEdgeCorner.add(
            new THREE.Line(
    ...
    private GenerateUserNameText(): THREE.Group {
    ...
    private GenerateIdentityBackground(): THREE.Group {
        const identityBackground = new THREE.Group;

        identityBackground.add(
            new THREE.Mesh(
                new THREE.PlaneGeometry(17, 2.7),
                new THREE.MeshBasicMaterial({ color: Colour.RetroWhite, side: THREE.DoubleSide, opacity: 0.1, transparent: true })
            ));
    ...
    private GenerateIdentityText(): THREE.Group {
    ...
    private GenerateNameSoftBorder(): THREE.Group {
        ...
        softBorder.add(
            new Wireframe(
                new LineSegmentsGeometry().fromEdgesGeometry(new THREE.EdgesGeometry(roundedRectangleGeometry)),
                new LineMaterial({ color: Colour.RetroWhite, linewidth: 3 })
            ));
