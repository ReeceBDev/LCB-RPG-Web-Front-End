...
export class EmergencyHexGrid {
    scene: THREE.Scene;
    localHexList = new Array<THREE.Object3D>;
    setTimeoutIDs: Array<number>; //Note: IDs dont need to be taken off once ran as no exception is thrown if removed.

    hexGridRows: number;
    hexGridColumns: number;


    constructor(
        setScene: THREE.Scene,
        loader: SVGLoader,
        postProcessing: EffectInstructions,
        {
            hexGridOffsetTopLeft = new THREE.Vector3(0, 0, 0),
            hexGridRows = 5,
            hexGridColumns = 5,
            isInvisible = false,
        }: {
            hexGridOffsetTopLeft: THREE.Vector3,
            hexGridRows: number,
            hexGridColumns: number,
            isInvisible: boolean,
        })
    ...
    public AnimateAppear() {
        ...
        const burstAmount: number = 3;
        const minBurstDelay: number = 0.6 / (burstAmount); //Total time
        const maxBurstDelay: number = 1.3 / (burstAmount); //Total time
        const burstCooldownDelay: number = 1.2; //Total time

        const minAppearDelay: number = 0.3 / (this.hexGridRows * this.hexGridColumns - burstAmount); //Total time
        const maxAppearDelay: number = 1.0 / (this.hexGridRows * this.hexGridColumns - burstAmount); //Total time

        this.setTimeoutIDs = new Array<number>; //Clear

        //Get the hex IDs
        let remainingHexIDs: number[] = (() => { 
        ...
        for (let i = 0; i < burstAmount; i++) {
            const selectionRand: number = randInt(0, remainingHexIDs.length - 1);
            const targetID: number = remainingHexIDs[selectionRand];
    ...
    {
        let xRelative: number = hexGridOffsetTopLeft.x;
        let yRelative: number = hexGridOffsetTopLeft.y;
        let zRelative: number = hexGridOffsetTopLeft.z;
        ...
        for (let currentColumn = 1; currentColumn <= hexGridRows; currentColumn++) {
            ...
            for (let currentRow = 1; currentRow <= hexGridColumns; currentRow++) {
                const newHex: THREE.Group = this.GenerateEmergencyHex(scene, loader, isInvisible);
    ...
    private GenerateEmergencyHex(scene: THREE.Scene, loader: SVGLoader, isInvisible: boolean = false): THREE.Group {
    ...
    private GenerateEmergencyHexOriginal(scene: THREE.Scene, loader: SVGLoader, isInvisible: boolean = false): THREE.Group {
        const group = new THREE.Group();
    ...
    private RunAppearAnimation(index: number) {
